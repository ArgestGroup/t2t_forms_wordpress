...
class T2TForms {
	...
	public function getJs() {
		return self::sendRequest(self::$SERVER . '/get/js', array (
			'addJQuery' => $this->addJQuery
		));
	}
	...
	public function getPaySystems() {
		...
		if (!$transport || !in_array($transport, array (
				'train',
				'bus'
			)))
			return '';
		...
		if (isset ($_REQUEST['do'])) {
			...
			switch ($_REQUEST['do']) {
				case 'autocomplete' :
					...
					break;
				case 'tripinfo' :
					$router = '/' . $lang . '/get/tripinfo';
					break;
				case 'loadmap' :
					$router = '/' . $lang . '/get/carmap';
					break;
				case 'getfio' :
					$router = '/' . $lang . '/invoice/getFio';
					break;
				case 'passitem' :
					$router = '/invoice/passItem.ejs';
					break;
				case 'passitemBus' :
					$router = '/invoice/passItemBus.ejs';
					break;
				case 'checkemail' :
					$router = '/' . $lang . '/get/checkemail';
					break;
				case 'tryreg' :
					$params = array ();
				...
				case 'forget_pwd' :
					...
					$params = array ();
					$params['email'] = $email;
					$url = self::$SERVER . '/' . $lang . '/get/forget';
					error_log($url);
					$request = self::sendRequest($url, $params);
					echo $request;
					break;
			}
	...
	public function getJsLinks() {
		return json_decode(self::sendRequest(self::$SERVER . '/get/js', array (
			'addJQuery' => $this->addJQuery,
			'in_json' => true
		)));
	}
